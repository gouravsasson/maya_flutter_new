...
class _TalkToMayaState extends State<TalkToMaya>
  ...
  Future<void> setAudioMode({required bool duringCall}) async {
    const platform = MethodChannel('audio_mode_channel');

    try {
      await platform.invokeMethod(
        duringCall ? 'setCallMode' : 'setDefaultMode',
      );
    } catch (e) {
      print("Audio mode error: $e");
    }
  }

  void _setupCallInterruptionHandler() async {
    _callService.onCallStarted = () async {
      if (!mounted || _session == null || _isMicMuted) return;

      print('CALL DETECTED → Muting Maya automatically');
      _wasMutedByCall = true;

      await setAudioMode(duringCall: true); // 🔥 Switch to OS call mode
    ...
    _callService.onCallEnded = () async {
      if (!mounted || _session == null || !_wasMutedByCall) return;

      print('CALL ENDED → Restoring Maya audio');
      await Future.delayed(const Duration(milliseconds: 600));

      await setAudioMode(duringCall: false); // 🔥 Restore normal audio route
            ...
            GestureDetector(
              ...
              child: AnimatedBuilder(
                ...
                builder: (_, __) => Transform.scale(
                  scale:
                      _orbScaleAnimation.value * _speakingPulseAnimation.value,
                  child: Container(
