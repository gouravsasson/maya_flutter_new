...
class _AudioCardPlayerState extends State<AudioCardPlayer>
    ...
    _audioPlayer.onPlayerStateChanged.listen((state) async {
      ...
      if (state == PlayerState.completed) {
        ...
        setState(() {
          ...
          _sourceReady =
              false; // will flip to true once onDurationChanged fires again
        });
  ...
  Future<void> _shareAudioFile() async {
    try {
      ...
      final filePath =
          '${tempDir.path}/shared_audio_${DateTime.now().millisecondsSinceEpoch}.mp3';
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text("Failed to share audio")));
      }
    }
  }
  ...
  Future<void> _seekRelative(Duration delta) async {
    ...
    final int targetMs = (position.inMilliseconds + delta.inMilliseconds).clamp(
      0,
        ...
        children: [
          ...
          SliderTheme(
            ...
            child: Slider(
              ...
              onChanged:
                  (duration == null || (duration?.inMilliseconds ?? 0) == 0)
                  ? null
...
class AudioWaveformPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final double startX =
        (size.width - (amplitudes.length * step) + spacing) / 2;
