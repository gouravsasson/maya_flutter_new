...
class AuthRepositoryImpl implements AuthRepository {
  ...
  Future<Either<Failure, User>> login({
    ...
    if (await networkInfo.isConnected) {
      try {
        print('🔐 Repository: Starting login process for $email');
    ...
    try {
      ...
      if (await networkInfo.isConnected) {
        try {
          ...
          print('⚠️ Repository: Server logout failed, continuing with local logout - ${e.toString()}');
          // Continue with local logout even if server logout fails
        }
      } else {
        print('ℹ️ Repository: No internet connection, performing local logout only');
      }
  ...
  Future<Either<Failure, User?>> getCurrentUser() async {
    try {
      ...
      if (user != null) {
        print('✅ Repository: Current user retrieved - ${user.firstName} ${user.lastName}');
        return Right(user);
      } else {
      ...
      print('❌ Repository: Unexpected error retrieving current user - ${e.toString()}');
  ...
  Future<Either<Failure, bool>> isLoggedIn() async {
    try {
      ...
      print('ℹ️ Repository: Login status check - Token: $hasToken, User: $hasUser, Result: $isLoggedIn');
      ...
      print('❌ Repository: Unexpected error checking login status - ${e.toString()}');
